...
게임 액션은 현재 revision을 함께 보내고, 서버는 일치할 때만 다음 revision으로 갱신한다. 초성 정답, 그림 제시어, 라이어 정보, 우노 손패, 다빈치 코드의 비공개 숫자처럼 사용자마다 달라야 하는 정보는 서버에서 투영한 뒤 전달한다.
그림 제한시간, 5초 정답 공개, 초성 힌트처럼 시간에 따라 바뀌는 상태는 서버 시각으로 전진시키고 테스트에서는 시각을 직접 주입한다.
